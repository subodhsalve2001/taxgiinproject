...
const { udyamdocsUpload } = require("../../../utils/upload")
const multiDocs = require("../../../models/upload-models/company/udyam_docs_upload_model")
exports.addDocController = async (req, res) => {

    try {
        udyamdocsUpload(req, res, async (err) => {
            if (err) {
            ...
            if (req.files.encorporationcert) {
                req.body.encorporationcert = `encorporationcert/${req.files.encorporationcert[0].filename}`
            }
            if (req.files.gstin) {
